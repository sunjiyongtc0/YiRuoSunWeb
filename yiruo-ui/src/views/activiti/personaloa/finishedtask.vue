<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="listQuery.taskName" placeholder="任务名称" clearable class="filter-input filter-item" style="width: 200px" @keyup.enter.native="handleFilter" />
    ...
    >
      ...
      <el-table-column label="任务开始时间" min-width="100px" prop="startTime">
        <template slot-scope="{row}">
          <span>{{ row.taskLocalVariables.startTime | formatDate }}</span>
        </template>
      </el-table-column>
      <el-table-column label="任务结束时间" min-width="100px" prop="endTime">
        <template slot-scope="{row}">
          <span>{{ row.taskLocalVariables.endTime | formatDate }}</span>
        </template>
      </el-table-column>
...
<script>
import { queryPersonalTask, getHighLightedProcessDiagram } from '@/api/activiti/activiti'
